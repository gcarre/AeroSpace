...
When you use this flag, `tiling`, `floating`, and `toggle-split` become invalid values for `<target-layout>`.
...
* `dwindle` - Recursively split the focused window. Dwindle can only be enabled on the workspace root node with `--root`
* `toggle-split` - In Dwindle, toggle the focused window's immediate split between side-by-side and top-and-bottom
...
* Switch the workspace to dwindle layout: +
`aerospace layout --root dwindle`

* Toggle the focused Dwindle split (equivalent to Hyprland's `togglesplit`): +
`aerospace layout toggle-split`
