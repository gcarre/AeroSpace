...
. <<layouts,Layout>> (Possible values: `tiles`, `accordion`; the workspace root additionally supports `dwindle`)
...
By default, AeroSpace uses the `dwindle` layout. Each new tiled window splits
the focused window into two. Wide regions split left/right, while tall regions
split top/bottom. Repeating this process produces a binary spiral similar to
Hyprland's Dwindle layout.

AeroSpace also provides 4 container layouts:
